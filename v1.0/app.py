...
def save_settings():
    ...
    try:
        ...
        if email_receiver is not None:
            setting = Settings.query.filter_by(key='email_receiver').first()
            if setting:
                setting.value = email_receiver
            else:
                setting = Settings(key='email_receiver', value=email_receiver)
                db.session.add(setting)
        ...
        if additional_emails is not None:
            setting = Settings.query.filter_by(key='additional_emails').first()
            if setting:
                setting.value = additional_emails
            else:
                setting = Settings(key='additional_emails', value=additional_emails)
                db.session.add(setting)
